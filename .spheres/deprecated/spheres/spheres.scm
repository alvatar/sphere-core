...
Usage: sspheres [command] [operand]
...
(define (install-cmd cmd opts args)
  ...
  (for-each (lambda (arg)
              (if (string=? "core" arg)
                  (die/error "Core Sphere is not intended to be installed with the 'sspheres' program.")))
            args)
  ;; Do the task
            ...
            (cond ((null? i)
                  ...
                  ((and (equal? sphere: (caar i))
                   ...
                   (if (file-exists? (string-append (path-expand "~~spheres/") target-id))
                       ...
                       ;; First try with Https protocol, then Git
                   ...
                   (if (or (member target-id args)
                           update)
                       (if (zero? (shell-command
                                   (string-append
                                    "cd " (path-expand "~~spheres/") target-id
                                    (if branch (string-append " && git checkout " branch))
                                    " && ssake")))
                           (set! results
                                 (append results `((,target-id "OK"))))
                           (begin
                             (set! results
                                   (append results `((error: ,target-id "\033[00;31mERROR -- Please contact Sphere maintainer and provide as much information as possible.\033[00m"))))
                             (leave)))
...
(define (unknown-cmd cmd opts args-sans-opts)
  (die/error "Unknown command:"
             cmd
             "To get a list of options, type 'sspheres help'"))
