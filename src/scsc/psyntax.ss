...
;;;
;;; mdh - $sc-put-cte is modified as a parameter function, to
;;;       support dynamic interception and delegation of its
;;;       implementation.
...
;;; should be sufficient to redefine syntax-expression to work for
;;; both old and new representations and syntax-wrap to return the
      ...
      ((_ (name id1 ...))
       (with-syntax
        ((expansion (map (lambda (datum)
                           (datum->syntax (syntax name) datum))
                         (##define-type-expand
                           'define-structure #f #f (map syntax->datum
                                                        (syntax (name id1 ...)))))))
        (syntax expansion))))))
...
;;; mdh - 
;;; the definition of generate-id is now dynamic to allow
;;; pluggable module loader implementations to provide their
;;; own implementations, on a per module loader basis.
...
;***     ((_ sym exp token) `($sc-put-cte ',sym ,exp ',token))))
    ((_ sym exp token)
     (build-source #f `(,(build-source #f `(,(build-source #f '$sc-put-cte)))
...
(define-syntax build-visit-only
 ...
 ; in implementations that support visit/revisit
  ...
  ;; mdh - visits are annotated with a visit-marker.
  ;;       the visit marker is interpreted at postlude time, and reverted
  ;;       back to a source marker before handing off to the gambit
  ;;       interpreter/compiler.
  (syntax-rules ()
    ((_ exp) (begin
               (vector-set! exp 0 visit-marker)
               exp))))
...
(define nonsymbol-id?
  (lambda (x)
    (and (syntax? x)
         (symbol? (unannotate (syntax-expression x))))))

(define id?
  (lambda (x)
    (cond
      ((symbol? x) #t)
      ((syntax? x) (symbol? (unannotate (syntax-expression x))))
...
(define-syntax id-sym-name
  (syntax-rules ()
    ((_ e)
     (let ((x e))
       (unannotate (if (syntax? x) (syntax-expression x) x))))))

(define id-marks
  (lambda (id)
    (if (syntax? id)
        (wrap-marks (syntax-wrap id))
        (wrap-marks top-wrap))))

(define id-subst
  (lambda (id)
    (if (syntax? id)
        (wrap-subst (syntax-wrap id))
        (wrap-marks top-wrap))))
...
(define id-sym-name&marks
  (lambda (x w)
    (if (syntax? x)
        (values
          (unannotate (syntax-expression x))
...
(define-syntax only-top-marked?
  (syntax-rules ()
    ((_ id) (same-marks? (wrap-marks (syntax-wrap id)) (wrap-marks top-wrap)))))
...
(module (gen-indirect-label indirect-label? get-indirect-label set-indirect-label!)
  (define-structure (indirect-label (label indirect-label-label set-indirect-label-label!)))
 ...
 ; ribcage guaranteed to be list-based
  (lambda (ribcage id label)
    (set-ribcage-symnames! ribcage
      (cons (unannotate (syntax-expression id))
  ...
  (lambda (ribcage new-marks id label)
    (set-ribcage-symnames! ribcage
      (cons (unannotate (syntax-expression id))
            (ribcage-symnames ribcage)))
    (set-ribcage-marks! ribcage
      (cons (join-marks new-marks (wrap-marks (syntax-wrap id)))
            (ribcage-marks ribcage)))
    (set-ribcage-labels! ribcage
      (cons label (ribcage-labels ribcage)))))
...
(define extend-ribcage-barrier!
 ...
 ; ribcage guaranteed to be list-based
  (lambda (ribcage killer-id)
    (extend-ribcage-barrier-help! ribcage (syntax-wrap killer-id))))
      ...
      (and new
           (let f ((new new))
             (cond
               ...
               ((same-marks? marks (join-marks new-marks (wrap-marks (syntax-wrap new)))) new)
               (else #f)))))))
...
(define resolved-id-var-name
  (lambda (id)
    (vector-ref
      (ribcage-labels (car (wrap-subst (syntax-wrap id))))
      0)))
  ...
  (define new-binding
    (lambda (sym marks token)
      (let ((loc (if (and (leave-implicit? token)
                     ...
                     sym
                     ;;; mdh - $generate-id is parameter function
                     (($generate-id) sym))))
...
(define id-var-name-loc&marks
  (lambda (id w)
    ...
    (cond
      ...
      ((syntax? id)
       (let ((sym (unannotate (syntax-expression id)))
             (w1 (syntax-wrap id)))
      ...
      (case type
        ...
        ((define-syntax-form)
         (let-values (((id rhs w) (parse-define-syntax e w ae)))
           (let ((id (wrap id w)))
             ...
             (let ((sym (id-sym-name id)))
               (let-values (((valsym bound-id) (top-id-bound-var-name sym (wrap-marks (syntax-wrap id)) top-ribcage)))
        ...
        (($import-form)
         (let-values (((orig only? mid) (parse-import e w ae)))
           ...
           (ct-eval/residualize2 ctem
             (lambda ()
               ;; provide hook for future module loader support.
               ;; $load-module is called when the mid is not defined in the cte.
               (let retry ((binding (lookup (id-var-name mid empty-wrap) null-env)))
                 (case (binding-type binding)
                   (($module)
                    ($update-module mid ctem rtem)
                    (do-top-import only? top-ribcage mid (interface-token (binding-value binding))))
                   ((displaced-lexical) (displaced-lexical-error mid))
                   (else 
                    (case ($load-module mid ctem rtem)
                      ((success) (retry (lookup (id-var-name mid empty-wrap) null-env)))
                      ((ignore) (build-data no-source (void)))
                      ((not-found) (syntax-error mid "unknown module"))))))))))
        ((alias-form)
         (let-values (((new-id old-id) (parse-alias e w ae)))
           (let ((new-id (wrap new-id w)))
             ...
             (let ((sym (id-sym-name new-id)))
               (let-values (((valsym bound-id) (top-id-bound-var-name sym (wrap-marks (syntax-wrap new-id)) top-ribcage)))
                 ...
                 (ct-eval/residualize2 ctem
                   (lambda ()
                     (build-cte-install
                       (make-resolved-id sym (wrap-marks (syntax-wrap new-id)) (id-var-name old-id w))
...
(define-structure (module-binding type id label imps val (exported module-binding-exported set-module-binding-exported!)))
              ...
              ((syntax? x)
               (let ((w (syntax-wrap x)))
                 (let ((ms (wrap-marks w)) (s (wrap-subst w)))
                   (make-syntax-object (syntax-expression x)
...
(define-syntax debug-when
  (syntax-rules ()
    ((_ s e1 e2 ...) (if (and (pair? (syntax-case-debug))
                              (memq 's (syntax-case-debug)))
                         (begin e1 e2 ...)))))
  ...
  (lambda (e c r mr w m?)
    ...
    (define ids/emitter
      (lambda (formals ids emitter ae template)
        (debug-when dsssl
                    (pp (vector 'ids/emitter ids 'template template)))
        (cond ((null? ids)
               (values (reverse formals) emitter))
              ((syntax? (car ids))
               (ids/emitter formals (cons (syntax->datum (car ids)) (cdr ids)) emitter ae (car ids)))
              ((annotation? (car ids))
              ...
              ((pair? (car ids))
               (ids/emitter formals (cons (car (car ids)) (cdr ids)) emitter ae
                            (if template 
                                (make-syntax-object (car (unannotate (syntax-expression template)))
                                                    (syntax-wrap template))
                                #f)))
              (else
    ...
    (define emit-formals
      (lambda (formals* formals vars emitter ae template)
        ...
        (cond ((null? formals) (reverse formals*))
               ...
               ((syntax? (car formals))
                (emit-formals formals* (cons (syntax->datum (car formals)) (cdr formals)) vars emitter ae (car formals)))
        ...
        (((id ...) e1 e2 ...)
         (let ((formals (syntax (id ...))))
           (let-values (((ids emitter) (ids/emitter '() formals 'rnrs #f #f)))
             (debug-when dsssl (pp (vector 'formals formals 'ids ids)))
  ...
  (lambda (x w fn)
    (if (top-marked? w)
        ...
        (let f ((x x))
          (cond
            ((syntax? x)
             (strip* (syntax-expression x) (syntax-wrap x) fn))
            ((pair? x)
...
;;; mdh - default implementation of $load-module and $reload-module
;;;       module loader functions preserve default psyntax behavior.
  (set! $load-module
        (lambda (mid ctem rtem) 'not-found))

  (set! $update-module
        (lambda (mid ctem rtem) (void)))

  (set! $include-file-hook
        (lambda (filename) (void)))

                                        ; must precede global-extends

;;; mdh - $sc-put-cte is a parameter function
  (set! $sc-put-cte
        (make-parameter
...
;;; mdh - gambit namespace support is removed/deprecated, thus, only 
;;;       the non-## named gambit symbols are exposed to the user.

;;; mdh - revisit c-define-type introduces bindings
...
;;; mdh - revisit c-define introduces bindings
  (global-extend 'core 'c-define
                 (lambda (e r mr w ae m?)
                   (attach-source ae (strip e w))))

;;; mdh - ##define is removed, to prevent users from using it as a
;;; DSSSL backdoor. this practice is likely defective as the syntax
;;; expander has no visibility of DSSSL bindings.
;;;
...
;;; mdh - ##namespace is removed because the sc#prop-table is global,
;;;       thus syntax definitions do not honor the namespaces in which
;;;       syntax is defined. this in turn may lead to defects and
;;;       unexpected behavior.
...
;;; mdh - provide ##begin as first-class begin-form
  (global-extend 'begin '##begin '())
             ...
             (let* ((interface (binding-value b))
               ...
               (vmap (lambda (x)
                       (let ((id (if (pair? x) (car x) x)))
                         (make-syntax-object
                          (syntax->datum id)
                          (let ((marks (join-marks new-marks (wrap-marks (syntax-wrap id))))) 
                            (make-wrap marks
    ...
    (define $import-help
      (lambda (orig import-only?)
        (lambda (r)
          ...
          (define prefix-add
            (lambda (prefix-id)
              (let ((prefix (symbol->string (syntax->datum prefix-id))))
                (lambda (id)
                  (datum->syntax id
                                        (string->symbol
          ...
          (define gen-mid
            (lambda (mid)
                                        ...
                                        ; the module id
;;; mdh - $generate-id is parameter function
              (datum->syntax mid (($generate-id) (id-sym-name mid)))))
...
;;; a file.
  ...
  (set! sc-expand (make-expander '(E) '(E)))
  ...
  (set! datum->syntax
        (lambda (id datum)
          (arg-check nonsymbol-id? id 'datum->syntax)
          (make-syntax-object
           datum
           (syntax-wrap id))))
      ...
      (lambda (e x-pat y-pat z-pat w r)
        (let f ((e e) (w w))
          (cond
           ...
           ((syntax? e) (f (syntax-expression e)
                                  (join-wraps w (syntax-wrap e))))
    ...
    (define match
      (lambda (e p w r)
        (cond
         ...
         ((syntax? e)
          (match*
           (unannotate (syntax-expression e))
           p
           (join-wraps w (syntax-wrap e))
           r))
    ...
    (set! $syntax-dispatch
          (lambda (e p)
            (cond
             ((eq? p 'any) (list e))
             ((syntax? e)
              (match* (unannotate (syntax-expression e))
                      p (syntax-wrap e) '()))
             (else (match* (unannotate e) p empty-wrap '()))))))

;; mdh - definitions formerly defined with define-syntax are now expressed by
;;       global-extend to prevent leakage into *top*.
...
;; mdh - revisit to use/extend gambit cond-expand implementation
   (global-extend
    'macro 'cond-expand
